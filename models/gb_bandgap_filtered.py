"""GB pipeline for bandgap prediction (filtered: bandgap > 0.1 eV)."""
import pickle, sys
sys.path.insert(0, "..")
...
if __name__ == "__main__":
    X, y = load_data(target="bandgap", bandgap_filter=0.1, **PATHS)
    results = run_nested_cv_with_holdout(X, y)
    with open("results/results_gb_bandgap_filtered.pkl", "wb") as f:
        pickle.dump(results, f)
    print("Saved to results/results_gb_bandgap_filtered.pkl")
